...
- [x] Player TV overlay contract: controls hidden by default, Center/OK reveals the unified control surface, inactivity hides it unless focus/interaction holds it open, Back closes the overlay first and falls through only when hidden, hidden controls retain no unreachable focus, favorite action moved into the overlay surface, and overlay state is keyed per playback generation. No second player/retry owner is introduced; recovery actions remain service-owned.
...
- [x] Move UI-facing presentation projection out of Compose: `GuidePresentation.kt` builds `GuideRowProjection`/`GuideCellProjection`/`GuideWindowPresentation` in the ViewModel publish path; Compose consumes preformatted row/cell identity, primary/secondary labels, programme time labels, `NO_GUIDE`/`SOURCE_CONFLICT` copy and focus detail labels. Geometry, theme colors, Dp calculations, current-time marker and actual focus state remain Compose concerns.
- [x] Keep channel ID + programme key focus identity, deterministic nearest/surviving fallback, Player → Back restoration, page transition semantics and safe focus under EPG/data invalidation; the projection feeds `GuideFocusChannel` from cell order.
- [x] Validate bounded performance against a realistic consumer window (bounded channel window, 6h programme window with existing adaptive narrowing, realistic density, mix of explicit stop and open-ended programmes, `NO_GUIDE` and source conflict) through ViewModel and presentation unit tests; no 50k full-grid benchmark.
- [x] Change `GuideWindowDao`/indexes/schema only with reproducible bounded hotspot evidence; no hotspot was measured, so no SQL change belongs to S6.
- [x] Full guide is never materialized for UI; truncated programme windows trigger only bounded narrowing or explicit `Incomplete` state; stale generations never publish UI state; static Guide consumes preformatted labels without per-frame formatting churn; long Russian labels survive projection intact with single-line ellipsis in Compose; D-pad focus identity is unchanged from the accepted Guide route.
...
- 2026-08-13: C0 merged as `main@2bc731ef` through PR #161.
- 2026-08-13: S6 presentation slice moves all Guide formatting/copy into `GuidePresentation.kt`; `GuideUiState.Content` publishes `GuideRowProjection`/`GuideCellProjection`/`GuideWindowPresentation` built in the ViewModel publish path, and `GuideRoute` renders preformatted labels. Focus identity, paging, bounded narrowing and stale-generation semantics are unchanged. Local evidence: guide unit suite green (Focus 8, Paging 5, Presentation 12, TimelineMath 3, ViewModel 8) and `app:tv:compileDebugKotlin` passes; Full/device runs are owned by CI per the user's no-heavy-local-runs instruction.
- 2026-08-13: S6 merged as `main@64ce4adb` through PR #162 (Full + product device matrix + benchmark dry run green).
- 2026-08-13: M4-R Player overlay slice: `PlayerContent` gains the TV overlay contract (`player-surface` fullscreen reveal target, `player-overlay` unified control surface with favorite/pause/stop/back, 6s inactivity auto-hide with interaction resets, BackHandler closing the overlay first). `PlayerFavoriteRoute` loses its floating top-end control and passes a `PlayerFavoriteAction` into the overlay; no second player/retry owner and no per-generation state leak (`remember` keyed on channel identity). Instrumentation contract updated (`PlayerOverlayJourneyTest` added, `PlayerHttpApprovalTest` reveals controls via Center before asserting focus). Device evidence: a third device iteration replaced `performClick` with D-pad focus navigation (Left from the primary action lands on favorite) plus Center/Enter activation — every keydown inside the overlay resets the auto-hide timer, so the activation is deterministic and the previous 6s auto-hide race with the click is gone; both API26 and API36 legs of the product device matrix are green (42/42 each). Two earlier test-design defects were corrected first — the second Espresso `pressBack` throws `NoActivityResumedException` by design once Back kills the activity, so the fall-through leg now dispatches through the activity `OnBackPressedDispatcher`, and raw key injection on an unfocused node cannot simulate TV Center activation.
