...
- 2026-08-13: M4-R Player overlay slice: `PlayerContent` gains the TV overlay contract (`player-surface` fullscreen reveal target, `player-overlay` unified control surface with favorite/pause/stop/back, 6s inactivity auto-hide with interaction resets, BackHandler closing the overlay first). `PlayerFavoriteRoute` loses its floating top-end control and passes a `PlayerFavoriteAction` into the overlay; no second player/retry owner and no per-generation state leak (`remember` keyed on channel identity). Instrumentation contract updated (`PlayerOverlayJourneyTest` added, `PlayerHttpApprovalTest` reveals controls via Center before asserting focus). Device evidence: API26 product matrix proved reveal/Back/auto-hide on device; two test-design defects were corrected — the second Espresso `pressBack` throws `NoActivityResumedException` by design once Back kills the activity, so the fall-through leg now dispatches through the activity `OnBackPressedDispatcher`, and the favorite activation uses `performClick` because key injection on an unfocused node cannot simulate TV Center activation (key activation on focused buttons remains covered by the approval and reveal tests).
