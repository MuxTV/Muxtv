...
**Interfaces:**
- Consumes composite revision identities: `(sourceId, revisionNumber)` from `source_revisions` and `epg_revisions`.
...
- [ ] **Step 1: Write RED migration/schema contracts** asserting v6→v7 preserves existing rows and creates exactly the expected PK/FKs/indices.
- [ ] **Step 2: Run** `:core:database:compileDebugAndroidTestKotlin` and migration tests; verify RED because v7/table/migration do not exist.
- [ ] **Step 3: Add entity** with fields:

```kotlin
@Entity(
    tableName = "epg_channel_matches",
    primaryKeys = [
        "epgSourceId", "epgRevisionNumber", "providerSourceId",
        "catalogRevisionNumber", "epgExternalChannelId",
    ],
)
...
Add composite cascading FKs to `EpgRevisionEntity(epgSourceId, epgRevisionNumber)` and `SourceRevisionEntity(providerSourceId, catalogRevisionNumber)` plus bounded-read indices.
- [ ] **Step 4: Add `MIGRATION_6_7`** creating only this table/indices; bump database to 7 and register migration.
- [ ] **Step 5: Run migration/schema tests GREEN**, export/commit Room schema 7.
...
- [ ] **Step 1: RED tests** for NFC/trim ID behavior, case-sensitive IDs, case/whitespace-insensitive names, punctuation preservation and no transliteration.
- [ ] **Step 2: RED decision tests** proving exact-ID candidate wins; one distinct canonical ID matches; multiple distinct canonical IDs are ambiguous; zero unresolved.
- [ ] **Step 3: Implement minimal normalization** with `java.text.Normalizer` and `Locale.ROOT` only for display names.
- [ ] **Step 4: Implement decision collapse** based on distinct canonical IDs, never row order.
...
- [ ] **Step 1: RED Android contracts**: exact ID beats duplicate names; duplicate exact names across distinct canonical IDs ambiguous; same name in another provider cannot match; renamed text remains exact-ID matched; same revision pair is idempotent.
- [ ] **Step 2: Add bounded DAO input reads** for active EPG channels and active provider rows scoped to one explicit provider source/revision.
- [ ] **Step 3: Implement ladder** `exact ID -> exact tvgName -> exact rawName -> unresolved`, collapsing every stage to distinct canonical IDs.
- [ ] **Step 4: Replace rows transactionally** only for the exact current revision pair. Store reason/candidate count only; never raw normalized strings.
- [ ] **Step 5: Detect active-revision movement during transaction** by re-reading producer revisions before commit; if changed, discard computed rows and return `Superseded`.
- [ ] **Step 6: Run Android contracts GREEN**.
...
- [ ] **Step 1: RED model bounds/validation tests**.
- [ ] **Step 2: Add immutable API models**; no persistence entities escape database module.
- [ ] **Step 3: Commit** `feat: define bounded epg now-next api`.
...
- [ ] **Step 1: RED tests** for current/next, hidden exclusion, revision switch, open-ended current programme, no-guide channel and strict request bounds.
- [ ] **Step 2: Add one bounded SQL path** joining requested canonical IDs → current revision-pair matches → active EPG programmes. Reject excessive IDs; never materialize the full guide.
- [ ] **Step 3: Implement effective end**: explicit stop when present, otherwise next programme start; an open-ended row without a subsequent programme does not become infinite.
- [ ] **Step 4: Return earliest future start/end that can change any requested projection as `nextBoundaryEpochMillis`.
- [ ] **Step 5: Run Android contracts GREEN**.
...
- [ ] **Step 1: RED orchestration tests** proving only successful durable publication schedules/reconciles affected relationships.
- [ ] **Step 2: Add store lookup** for linked EPG source IDs, bounded per provider source.
- [ ] **Step 3: Invoke matching after durable completion is known `APPLIED`; matching failure must not roll back already-published catalog/EPG revision and must surface as typed/recoverable reconciliation state rather than replaying network refresh.
...
**Files:**
- Extend `catalog/api` guide contract with a lightweight invalidation signal if needed by #29.
- Implement from Room active-revision changes plus the returned next boundary; no periodic polling loop.
- Test deterministic boundary transitions.

- [ ] **Step 1: RED test** that unchanged revision pair emits no spurious data invalidation.
- [ ] **Step 2: RED test** that catalog or EPG active revision switch invalidates immediately.
- [ ] **Step 3: RED test** that time invalidation is scheduled only at the earliest next programme boundary.
- [ ] **Step 4: Implement minimal Flow/timer owner** outside Compose.
- [ ] **Step 5: Commit** `feat: expose epg revision and boundary invalidation`.
...
- [ ] **Step 1: Integration RED** on missing end-to-end binding.
- [ ] **Step 2: Prove previous-good catalog/EPG stays active through malformed input, cancellation and supersede.
- [ ] **Step 3: Prove diagnostics contain codes/counts but no raw provider/search/programme/credential values.
- [ ] **Step 4: Exact-head Full + API26/API36 matrix.
- [ ] **Step 5: Close #71 and #28 only after exact-head evidence; update repository truth in a separate documentation commit/PR.
