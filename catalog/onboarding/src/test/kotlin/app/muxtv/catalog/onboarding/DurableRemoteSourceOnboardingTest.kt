...
class DurableRemoteSourceOnboardingTest {
    ...
    @Test
    fun restoreLatestPreparedReturnsOnlyOpaqueTokenAndSanitizedEndpoint() = runTest {
        val older = pending(token(20), createdAt = 1_000L, expiresAt = 100_000L)
        val latestToken = token(21)
        val latest = pending(latestToken, createdAt = 2_000L, expiresAt = 100_000L)
        val registry = FakeRegistry(rows = mutableListOf(older, latest))
        val onboarding = DurableRemoteSourceOnboarding(
            delegate = FakeOnboarding(),
            registry = registry,
        ...
        assertThat(restored.toString()).doesNotContain(latestToken.value)
    }

    @Test
    fun restoreLatestPreparedDropsInvalidOpaqueId() = runTest {
        val invalid = PendingSourcePreparation(
            preparationId = "not-a-credential-id",
            scheme = "https",
            host = "example.com",
            createdAtEpochMillis = 2_000L,
            expiresAtEpochMillis = 100_000L,
        )
        val registry = FakeRegistry(rows = mutableListOf(invalid))
        val onboarding = DurableRemoteSourceOnboarding(
            delegate = FakeOnboarding(),
            registry = registry,
            currentTimeMillis = { 10_000L },
    ...
    @Test
    fun completedActivationIsNotRewrittenByRegistryCleanupCancellation() = runTest {
        val token = token(33)
        val activated = RemoteSourceActivationResult.Activated(
            sourceId = "source-opaque",
            revisionNumber = 1,
            previousRevisionNumber = 0,
            entryCount = 10,
        ...
        val registry = FakeRegistry(
            rows = mutableListOf(pending(token, expiresAt = 100_000L)),
            removeFailure = CancellationException("registry cancellation"),
        )
        val onboarding = DurableRemoteSourceOnboarding(
            delegate = FakeOnboarding(activateResult = activated),
            registry = registry,
            currentTimeMillis = { 3_000L },
        )

        val result = onboarding.activate(token, "Primary")

        assertThat(result).isSameInstanceAs(activated)
...
private class FakeRegistry(
    ...
    override suspend fun remove(preparationId: String): Boolean {
        ...
        return rows.removeAll { it.preparationId == preparationId }
    }
