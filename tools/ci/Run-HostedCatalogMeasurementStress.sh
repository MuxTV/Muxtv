...
if [[ "$MUXTV_EXPECTED_AVD" != "MuxTV_TV_CURRENT_API36" ]]; then
  echo "M0 catalog measurement stress is defined only on canonical API36, got $MUXTV_EXPECTED_AVD." >&2
  exit 1
fi
...
./gradlew \
  ...
  -Pandroid.testInstrumentationRunnerArguments.measurementRunnerLabel=github-hosted-linux-api36-m0-v4-stress \
  ...
  -Pandroid.testInstrumentationRunnerArguments.measurementOutputName=catalog-database-measurement-m0-stress.json
...
python3 - "${result_logs[0]}" "$MUXTV_MEASUREMENT_EVIDENCE/catalog-database-measurement-m0-stress.json" "$MUXTV_SOURCE_COMMIT" <<'PY'
import base64
import json
import pathlib
import re
...
if report.get("thresholdApplied") is not False:
    raise SystemExit("M0 stress evidence must remain thresholdApplied=false.")
if report.get("sourceCommit") != expected_sha:
    raise SystemExit("Catalog measurement report sourceCommit does not match exact workflow head.")
if report.get("runnerLabel") != "github-hosted-linux-api36-m0-v4-stress":
    raise SystemExit("Catalog measurement report runnerLabel does not match the manual v4 stress methodology.")
...
if workload.get("entryCount") != 50000 or workload.get("measuredIterations") != 5:
    raise SystemExit("Catalog measurement workload is not the manual 50k/5 stress contract.")
if workload.get("warmupIterations") != 0:
    raise SystemExit("Hosted M0 stress evidence must use zero warmup iterations.")
...
if not all(len(operation.get("rawSamples") or []) == 5 for operation in operations):
    raise SystemExit("Catalog measurement stress report does not contain five samples per operation.")

report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print("Catalog database M0 manual stress report accepted.")
...
cat > "$MUXTV_MEASUREMENT_EVIDENCE/m0-catalog-measurement-stress-summary.txt" <<EOF
status=passed
sourceCommit=$MUXTV_SOURCE_COMMIT
avd=$MUXTV_EXPECTED_AVD
api=36
mode=stress
runnerLabel=github-hosted-linux-api36-m0-v4-stress
methodVersion=4
entryCount=50000
warmups=0
iterations=5
...
echo "Manual hosted M0 catalog stress completed on $MUXTV_EXPECTED_AVD; 50k x 5; methodVersion=4; thresholdApplied=false; claimEligible=false."
