...
if (-not (Test-Path -LiteralPath $workflowPath -PathType Leaf)) {
    throw "Hosted U0 workflow is missing: $workflowPath"
}
if (-not (Test-Path -LiteralPath $entrypointPath -PathType Leaf)) {
    throw "Hosted U0 entrypoint is missing: $entrypointPath"
}
...
function Require-Token {
    ...
    if ($Text.IndexOf($Token, [System.StringComparison]::Ordinal) -lt 0) {
        throw "$Owner is missing required hosted U0 token: $Token"
    }
}
...
function Reject-Token {
    ...
    if ($Text.IndexOf($Token, [System.StringComparison]::Ordinal) -ge 0) {
        throw "$Owner still contains removed self-hosted U0 ownership token: $Token"
    }
}
...
foreach ($token in @(
    ...
    'New-TvAvd',
    'Start-TvEmulator',
    '.github/ui-characterization/run.request'
)) {
