...
try {
    ...
    $pendingGate = $pendingRequired.gates["scope.alpha_dependencies"]
    ...
    Assert-ValidatorFailure -Name "private-machine-absolute-path" -Manifest $privateMachinePath
...
if ($failures.Count -gt 0) {
    $message = "Alpha evidence validator contract failed." + [Environment]::NewLine +
