...
class EpgMatchingMigration6To7Test {
    ...
    @Test
    fun v7MatchRowsCascadeWhenCatalogRevisionIsPruned() = runBlocking {
        val version6 = migrationHelper.createDatabase(6)
        version6.execSQL("PRAGMA foreign_keys = ON")
        insertVersion6ProducerFixture(version6)
        version6.close()
        ...
        migrated.execSQL(
            "DELETE FROM source_revisions WHERE sourceId = 'source-1' AND revisionNumber = 1",
        )
        assertSingleLong(migrated, "SELECT COUNT(*) FROM epg_channel_matches", 0)
        migrated.close()
    }

    private fun insertMatchedRow(connection: SQLiteConnection) {
        connection.execSQL(
            """
