...
class GuideProgrammeWindowRepositoryTest {
    ...
    fun setUp() = runTest {
        ...
        publishMatches(
            ...
            matches = listOf(matched(EPG_SOURCE_TWO, EXTERNAL_C_TWO, CHANNEL_C)),
        )
    }
    ...
    fun programmeWindowAppliesOverlapMembershipStatesAndEffectiveEnds() = runTest {
        ...
        assertThat(window.channels.map { it.canonicalChannelId })
            .containsExactlyElementsIn(CHANNELS).inOrder()
        ...
        assertThat(channelA.programmes.map { it.title })
            .containsExactly("A current", "A middle", "A trailing").inOrder()
        ...
        assertThat(channelD.programmes.map { it.title })
            .containsExactly("D open", "D next").inOrder()
    ...
    fun programmeWindowUsesExtraRowForExplicitTruncation() = runTest {
        ...
        assertThat(window.channels.single().programmes.map { it.title })
            .containsExactly("A current", "A middle").inOrder()
    }
    ...
    private suspend fun publishMatches(epgSourceId: String, matches: List<EpgChannelMatchEntity>) {
        val snapshot = requireNotNull(database.epgMatchingDao().relationSnapshot(epgSourceId))
        assertThat(database.epgMatchingDao().replaceIfCurrent(snapshot, matches))
            .isEqualTo(EpgMatchPublicationResult.Applied)
    }

    private fun matched(epgSourceId: String, externalId: String, canonicalChannelId: String) =
    ...
    private companion object {
        ...
        val CHANNELS = listOf(CHANNEL_A, CHANNEL_B, CHANNEL_C, CHANNEL_D, CHANNEL_E, CHANNEL_F)
    }
}
