...
class GuideChannelWindowRepositoryTest {
    ...
    fun setUp() = runTest {
        ...
        sourceStore.upsertSource(SourceDefinition(id = SOURCE_ID, name = "Guide source"))
        stageRevision(
            revisionNumber = 1,
            entries = listOf(
    ...
    fun keysetPagesAreDeterministicAndRevisionSafe() = runTest {
        val first = repository.getChannelWindow(GuideChannelWindowQuery(PROFILE_ID, limit = 2))
        assertThat(first.channels.map { it.channelId }).containsExactly(CHANNEL_A, CHANNEL_E).inOrder()
        ...
        val second = repository.getChannelWindow(
            GuideChannelWindowQuery(PROFILE_ID, after = first.nextCursor, limit = 2),
        )
        assertThat(second.channels.map { it.channelId }).containsExactly(CHANNEL_C, CHANNEL_D).inOrder()
        assertThat(second.isTruncated).isFalse()
        assertThat(second.nextCursor).isNull()
        val beforeSwap = (first.channels + second.channels).map { it.channelId }
        assertThat(beforeSwap).doesNotContain(CHANNEL_B)
        assertThat(beforeSwap).doesNotContain(CHANNEL_F)

        activateRevision(2, 2)

        val afterSwap = repository.getChannelWindow(GuideChannelWindowQuery(PROFILE_ID, limit = 10))
        assertThat(afterSwap.channels.map { it.channelId }).containsExactly(CHANNEL_D, CHANNEL_F).inOrder()
        assertThat(afterSwap.channels.map { it.channelId })
            .containsNoneOf(CHANNEL_A, CHANNEL_B, CHANNEL_C, CHANNEL_E)

        val staleCursorContinuation = repository.getChannelWindow(
            GuideChannelWindowQuery(PROFILE_ID, after = first.nextCursor, limit = 10),
        )
        assertThat(staleCursorContinuation.channels.map { it.channelId })
            .containsExactly(CHANNEL_D, CHANNEL_F).inOrder()
    }

    private suspend fun stageRevision(revisionNumber: Long, entries: List<StagedCatalogEntry>) {
        sourceStore.beginRevision(SOURCE_ID, revisionNumber, revisionNumber * 1_000L)
        sourceStore.stageBatch(SOURCE_ID, revisionNumber, entries)
    }

    private suspend fun activateRevision(revisionNumber: Long, parsedEntries: Int) {
        assertThat(
            sourceStore.activate(
                sourceId = SOURCE_ID,
                revisionNumber = revisionNumber,
                activatedAtEpochMillis = revisionNumber * 1_000L + 500L,
                statistics = SourceRevisionStatistics(parsedEntries, 0, 0),
            ),
